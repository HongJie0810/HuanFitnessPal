        ...
        if ($stmt = $conn->prepare($availability_query)) {
            ...
            if ($availability_result->num_rows > 0) {
                ...
                if ($insert_stmt = $conn->prepare($insert_query)) {
                    $nutritionist_category = "Dietitian Nutritionist";
                    ...
                    if ($insert_stmt->execute()) {
                        ...
                        $payment_query = "INSERT INTO payment_information (invoice_number, user_id, payment_method, payment_date, payment_category, payment_amount) VALUES (?, ?, ?, ?, ?, ?)";
                        
                        if ($payment_stmt = $conn->prepare($payment_query)) {
                            $payment_stmt->bind_param("sissss", $invoice_number, $user_id, $payment_method, $payment_date, $payment_category, $payment_amount);
...
$nutritionist_sql = "SELECT Nutritionist_ID, Name FROM huan_fitness_nutritionist WHERE Category = 'Dietitian' ORDER BY Name";
...
<body>
    ...
    <form id="appointmentForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        <h2>Appointment with Dietetian Nutritionist</h2>
        ...
        <div class="button-group">
            ...
            <a href="receipt.php" class="back-btn">Back</a>
        </div>
    </form>
</body>
</html>